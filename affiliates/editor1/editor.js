...
this.btnList = {
    ...
    "insertimage2":   ['InsertImageGalery',    'Insert Image from galery','editor_action(this.id)',  'ed_image2.gif'],
   ...
   // "about":        ['about',                'About this editor',  'editor_about(\''+objname+'\')',  'ed_about.gif'],
    ...
    "cleanhtml":         ['custom1',       'Clean HTML from Word',   'editor_action(this.id)',  'clean_html.gif'],
    ...
    "dirrtl":         ['custom6',         'Direction Right-To-Left',  'editor_action(this.id)',  'he.gif'],
    "dirltr":         ['custom7',         'Direction Left-To-Right',  'editor_action(this.id)',  'en.gif'],
...
function editor_action(button_id) {
  ...
  // Custom1
  else if (cmdID == 'custom1') {  // insert some text from a popup window
   //editdoc.execCommand('Paste') 
   obj=eval('form1.'+objname) 
   var  CleanHTML = showModalDialog(_editor_url + "popups/cleaner.html", obj, "help: no; status: no; dialogWidth:300pt; dialogHeight:200pt; margin: 0");
    if (CleanHTML) {
     editdoc.execCommand('SelectAll'); 
     editdoc.execCommand('Delete'); 
     editor_insertHTML(objname, CleanHTML); 
                    }
  }

// Custom6
  else if (cmdID == 'custom6') {  // insert dir=rtl
    var editdoc = editor_obj.contentWindow.document;
    sRange  = editdoc.selection.createRange();
    var sHtml   = sRange.htmlText;
    editor_insertHTML(objname, "<span dir=rtl>"+sHtml+"</span>");
                    }
   
  // Custom7
  else if (cmdID == 'custom7') {  // insert dir=ltr
    var editdoc = editor_obj.contentWindow.document;
    sRange  = editdoc.selection.createRange();
    var sHtml   = sRange.htmlText;
    editor_insertHTML(objname, "<span dir=ltr>"+sHtml+"</span>");
                    }     
                    
  // Insert Image Galery popup
  else if (cmdID == 'InsertImageGalery') {
   locat=document.location.href
   if ((locat.indexOf("update-businesses.asp")!=-1)||(locat.indexOf("update-busproducts.asp")!=-1)){
	if (locat.indexOf("BusID=")!=-1){
	BusId=locat.substring(locat.indexOf("BusID=")+6,locat.length)
	x=window.open (_editor_url +'popups/image_galery.asp?objname='+objname+'&catId='+BusId,'newwin','top=50,left=50,width=400,height=600,scrollbars=yes')
	                             }
	else
	alert("First create a new Record, after it you can use image Galery.") 
                                                 } 
   else
   x=window.open (_editor_url +'popups/image_galery.asp?objname='+objname,'newwin','top=50,left=50,width=400,height=600,scrollbars=yes')
  }
...
function editor_updateToolbar(objname,action) {
  ...
  if (action == "enable" || action == "disable") {
    ...
    for (var idxN in tbItems) {
      ...
      if (cmdID == "htmlmode" || cmdID == "showhelp" || cmdID == "popupeditor") { continue; } // don't change these buttons
